...
VOLUME_NAME="EPD Editor Installer"
...
notarize() {
  ...
  printf "\nSigning EPD Editor executable at runtime...\n"
  codesign -f -v --deep --entitlements "$ENTITLEMENTS" --timestamp \
   --options runtime -i "$APP_ID" -s "$APP_CERT" "${APP}/Contents/MacOS/EPD Editor"
...
Create Mac distributions for the EPD Editor.
...
if [ "$1" = "pkg" ] || [ "$1" = "upload" ]; then
  while true; do
      printf "\n"
      read -rp "Do you wish to upload EPD Editor to the App Store? (Y/n)" answer
      case $answer in
          Y ) upload_pkg; break;;
          n ) exit;;
